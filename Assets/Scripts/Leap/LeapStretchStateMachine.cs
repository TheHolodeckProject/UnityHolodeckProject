...
public class LeapStretchStateMachine : MonoBehaviour
{
    ...
    private bool moving;
    enum State { Idle, StartStretch, Stretch, DoubleStretch, ExitStretch };
    private State currentState;

    // Use this for initialization
    void Start()
    {
        dualCorners = new string[2];
        pinching = false;
        onPinch = false;
        stretching = false;
    ...
    void Update()
    {
        //If something is being moved, don't allow stretching
        if (LeapMoveStateMachine.moving == true)
            return;
        GetHandInfo();
        ...
        {                
            ...
            case State.StartStretch:
                ...
                if (stretchHands == 1)
                    ...
                    currentState = State.DoubleStretch;
                break;
            case State.Stretch:
                    LeapDetectPinch();
                    if (onPinch)
            ...
            case State.ExitStretch:
                    StopStretching();
                break;
            case State.DoubleStretch:
            Debug.Log("Double Stretch?");    
            // ??? Double stretching needs access to both hand positions, so let a separate DoubleStretch script, placed on the stretchablecube, run until it changes this script's currentState to State.ExitStretch?
                break;
        }
    }
    ...
    void CheckPinchPosition()
    {
        ...
        for (int j = 0; j < closeThings.Length; ++j)
        {
            ...
            {
                currentState = State.StartStretch;
    ...
    void StopStretching()
    {
        stretching = false;
        stretchHands = stretchHands - 1;
        //Debug.Log("Exiting Stretch state. Stretchhands is now " + stretchHands);
        currentState = State.Idle;
    }
}