...
///
/// </summary>
public class OVRStereoCameras : MonoBehaviour
{
	...
	private GameObject		LeftCamera 			= null;
	private GameObject		RightCamera 		= null;
	private GameObject      GUIRenderLeftEye  	= null;
	private GameObject      GUIRenderRightEye  	= null;
	private RenderTexture	GUIRenderTexture 	= null;
	private WebCamTexture 	LeftCameraTexture 	= null;
	private WebCamTexture	RightCameraTexture	= null;
	...
	void Start()
	{
		...
		GUIRenderLeftEye = GameObject.Instantiate(Resources.Load("OVRGUIObjectMain")) as GameObject;
		GUIRenderLeftEye.name = "OVRLeftGUI";
		GUIRenderRightEye = GameObject.Instantiate(Resources.Load("OVRGUIObjectMain")) as GameObject;
		GUIRenderRightEye.name = "OVRRightGUI";

		LeftCamera = GameObject.Find ("CameraLeft");
		RightCamera = GameObject.Find ("CameraRight");

		if((GUIRenderLeftEye != null) && (GUIRenderRightEye != null))
		{
			// Chnge the layer
			GUIRenderLeftEye.layer = LayerMask.NameToLayer("LeftEye");
			GUIRenderLeftEye.transform.rotation = Quaternion.Euler (0, 0, -90) * GUIRenderLeftEye.transform.rotation;

			GUIRenderRightEye.layer = LayerMask.NameToLayer("RightEye");
			GUIRenderRightEye.transform.rotation = Quaternion.Euler (0, 0, 90) * GUIRenderRightEye.transform.rotation;
		...
		if(GUIRenderTexture != null && GUIRenderLeftEye != null && GUIRenderRightEye != null)
		{
			LeftCameraTexture = new WebCamTexture("UI325xLE-C_4102750498");
			//transform.rotation = Quaternion.Euler (0, 0, 90) * transform.rotation;
			LeftCameraTexture.Play();

			RightCameraTexture = new WebCamTexture("UI325xLE-C_4102750497");
			//transform.rotation = Quaternion.Euler (0, 0, 90) * transform.rotation;
			RightCameraTexture.Play();

			GUIRenderLeftEye.renderer.material.mainTexture = LeftCameraTexture;
			GUIRenderRightEye.renderer.material.mainTexture = RightCameraTexture;
			...
			if(CameraController != null)
            {
                ...
                Vector3 lsLeft = GUIRenderLeftEye.transform.localScale;
				Vector3 lpLeft = GUIRenderLeftEye.transform.localPosition;
				Quaternion lrLeft = GUIRenderLeftEye.transform.localRotation;

				Vector3 lsRight = GUIRenderRightEye.transform.localScale;
				Vector3 lpRight = GUIRenderRightEye.transform.localPosition;
				Quaternion lrRight = GUIRenderRightEye.transform.localRotation;

                // Attach the GUI object to the camera
                CameraController.AttachGameObjectToStereoCamera(ref GUIRenderLeftEye, "Left");
				CameraController.AttachGameObjectToStereoCamera(ref GUIRenderRightEye, "Right");
                // Reset the transform values (we will be maintaining state of the GUI object
                // in local state)

				GUIRenderLeftEye.transform.localScale = lsLeft;
                GUIRenderLeftEye.transform.localRotation = lrLeft;

				GUIRenderRightEye.transform.localScale = lsRight;
				GUIRenderRightEye.transform.localRotation = lrRight;
                ...
                Transform guiParentLeftEye = GUIRenderLeftEye.transform.parent;
                if (guiParentLeftEye != null)
                {
                    OVRCamera ovrCameraLeft = guiParentLeftEye.GetComponent<OVRCamera>();
                    if (ovrCameraLeft != null && ovrCameraLeft.RightEye)
                        ipdOffsetDirection = -1.0f;
                }

                float ipd = 0.0f;
                CameraController.GetIPD(ref ipd);
                lpLeft.x += ipd * 0.5f * ipdOffsetDirection;
                GUIRenderLeftEye.transform.localPosition = lpLeft;

				float ipdOffsetDirection2 = 1.0f;
				Transform guiParentRightEye = GUIRenderRightEye.transform.parent;
				if (guiParentRightEye != null)
				{
						ipdOffsetDirection2 = -1.0f;
				}
				
				float ipd2 = 0.0f;
				CameraController.GetIPD(ref ipd2);
				lpRight.x += ipd2 * 0.5f * ipdOffsetDirection2;
				GUIRenderRightEye.transform.localPosition = lpRight;

				GUIRenderLeftEye.SetActive(true);
                GUIRenderRightEye.SetActive(true);
            }
		}
	...
	{		
		if(Input.GetKeyDown(KeyCode.B)) {
			LeftCamera.transform.localScale += new Vector3(0.05f,0.05f,0.0f);
			RightCamera.transform.localScale += new Vector3(0.05f,0.05f,0.0f);
		}
		...
		if(Input.GetKeyDown(KeyCode.P)) {
			LeftCamera.transform.localScale += new Vector3(0.05f,0.00f,0.0f);
			RightCamera.transform.localScale -= new Vector3(0.05f,0.00f,0.0f);
		}
	...
	{	
		...
		/*if(GUIRenderTexture != null && GUIRenderLeftEye.activeSelf && GUIRenderRightEye.activeSelf)
		{
...
*/		
		...
		if (GUIRenderLeftEye.activeSelf && GUIRenderRightEye.activeSelf)
		{
	...
	void GUIShowLevels()
	{
		...
		{   
			...
			GUI.color = new Color(0, 0, 0, 0.5f);
  			GUI.DrawTexture( new Rect(0, 0, Screen.width/2, Screen.height ), LeftCameraTexture );
